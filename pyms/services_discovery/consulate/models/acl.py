...
def _validate_link_array(value, model):  # pylint: disable=unused-argument
...
def _validate_service_identities(value, model):  # pylint: disable=unused-argument
    ...
    """
    return all(["ServiceName" in service_identity for service_identity in value])
...
class ACL(base.Model):
    ...
    __attributes__ = {
        "id": {"key": "ID", "type": uuid.UUID, "cast_from": str, "cast_to": str},
        "name": {"key": "Name", "type": str},
        "type": {"key": "Type", "type": str, "enum": {"client", "management"}, "required": True},
